...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        presentDepth = 0
        presentAgentInd = 0
        val = self.value(gameState, presentAgentInd, presentDepth)
        return val[0]

    def value(self, gameState, presentAgentInd, presentDepth):

            noOfAgents = gameState.getNumAgents()
            if noOfAgents <= presentAgentInd:
                presentAgentInd = 0
                presentDepth += 1

            if self.depth == presentDepth:
                return self.evaluationFunction(gameState)

            return self.getValue(gameState, presentAgentInd, presentDepth, presentAgentInd)

            # if 0 == presentAgentInd:
            # else:
            #     return self.minValue(gameState, presentAgentInd, presentDepth, presentAgentInd)

    def getValue(self, gameState, currentAgentIndex, presentDepth, presentAgentInd):

            v = ("unknown", -1 * float("inf")) if presentAgentInd == 0 \
                else ("unknown", float("inf"))

            # if gameState.isWin() or gameState.isLose() or presentDepth == self.depth:
            #if presentDepth == self.depth:
            ...
            for action in gameState.getLegalActions(currentAgentIndex):
                ...
                gamState = gameState.generateSuccessor(currentAgentIndex, action)
                updatedVal = self.value(gamState, currentAgentIndex + 1, presentDepth)

                if type(updatedVal) is tuple:
                    updatedVal = updatedVal[1]


                if presentAgentInd == 0:
                    maxValue = max(v[1], updatedVal)
